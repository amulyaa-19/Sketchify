...
export class Game {
  ...
  private drawCurrentPencilPath() {
    if (this.currentPencilPath.length < 2) return;
    this.ctx.beginPath();
    this.ctx.strokeStyle = "rgba(255, 255, 255)";
    for (let i = 1; i < this.currentPencilPath.length; i++) {
      const prev = this.currentPencilPath[i - 1];
      const curr = this.currentPencilPath[i];
  ...
  mouseUpHandler = (e: { clientX: number; clientY: number }) => {
    ...
    if (this.selectedTool === "pencil" && this.currentPencilPath.length > 1) {
      const shape: Shape = {
        type: "pencil",
